...
In **Complet (full) mode**, the GM PC is just a browser client. The Room Controller Pi (192.168.2.10) runs everything (Node.js, MQTT, WebSocket, media storage). The GM PC connects to `http://<room-controller-ip>:<port>` and displays the dashboard. No server-side software runs on the GM PC.
